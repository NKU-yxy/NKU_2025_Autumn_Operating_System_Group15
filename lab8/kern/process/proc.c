...
load_icode(int fd, int argc, char **kargv)
{
    ...
    /* Minimal ELF section/symbol definitions used to locate __global_pointer$ */
    struct elf64_shdr
    {
        uint32_t sh_name;
        uint32_t sh_type;
    ...
    /* Locate __global_pointer$ in the ELF symbol table so gp starts correctly. */
    if (elf.e_shoff != 0 && elf.e_shnum != 0 && elf.e_shentsize == sizeof(struct elf64_shdr))
    {
        size_t shdrs_size = elf.e_shnum * sizeof(struct elf64_shdr);
        struct elf64_shdr *shdrs;
        if ((shdrs = kmalloc(shdrs_size)) == NULL)
        {
            ret = -E_NO_MEM;
            goto bad_mm;
        }
        if ((ret = load_icode_read(fd, shdrs, shdrs_size, elf.e_shoff)) != 0)
        {
            kfree(shdrs);
            goto bad_mm;
        }
        ...
        {
            if (shdrs[i].sh_type != SHT_SYMTAB || shdrs[i].sh_entsize != sizeof(struct elf64_sym))
            {
                continue;
            }
            struct elf64_shdr *str_sh = &shdrs[shdrs[i].sh_link];
            size_t symtbl_size = shdrs[i].sh_size;
            size_t strtbl_size = str_sh->sh_size;
            struct elf64_sym *syms = NULL;
            char *strtab = NULL;

            if ((syms = kmalloc(symtbl_size)) == NULL || (strtab = kmalloc(strtbl_size)) == NULL)
            {
                ret = -E_NO_MEM;
                kfree(syms);
                kfree(strtab);
            ...
            if ((ret = load_icode_read(fd, syms, symtbl_size, shdrs[i].sh_offset)) != 0 ||
                (ret = load_icode_read(fd, strtab, strtbl_size, str_sh->sh_offset)) != 0)
            {
                kfree(syms);
                kfree(strtab);
                kfree(shdrs);
            ...
            size_t sym_count = symtbl_size / sizeof(struct elf64_sym);
    ...
    {
        if ((ret = load_icode_read(fd, &ph, sizeof(ph), elf.e_phoff + i * sizeof(struct proghdr))) != 0)
        {
        ...
        uintptr_t la, file_end = ph.p_va + ph.p_filesz, mem_end = ph.p_va + ph.p_memsz;
        for (la = ph.p_va; la < mem_end; la += PGSIZE)
        {
            struct Page *page;
            ...
            size_t page_off = PGOFF(la);
            memset(kva, 0, PGSIZE);

            if (la < file_end)
            {
                size_t off = la - ph.p_va;
                size_t size = file_end - la;
                size_t copy = PGSIZE - page_off;
                if (size < copy)
                {
                    copy = size;
                }
                if ((ret = load_icode_read(fd, kva + page_off, copy, ph.p_offset + off)) != 0)
                {
                    goto bad_cleanup_mmap;
                }
            }
    ...
    for (int i = argc - 1; i >= 0; i--)
    {
        ...
        struct Page *page = get_page(mm->pgdir, sp, NULL);
        assert(page != NULL);
        memcpy(page2kva(page) + PGOFF(sp), kargv[i], len);
        uargv[i] = sp;
    }

    sp = ROUNDDOWN(sp, sizeof(uintptr_t));
    sp &= ~((uintptr_t)0xF); // keep stack 16-byte aligned for ABI safety
    sp -= (argc + 1) * sizeof(uintptr_t);
    argv_user = sp;

    for (int i = 0; i < argc; i++)
    {
        uintptr_t dst = argv_user + i * sizeof(uintptr_t);
        struct Page *page = get_page(mm->pgdir, dst, NULL);
        assert(page != NULL);
        memcpy(page2kva(page) + PGOFF(dst), &uargv[i], sizeof(uintptr_t));
    }
    {
        uintptr_t zero = 0;
        uintptr_t dst = argv_user + argc * sizeof(uintptr_t);
        struct Page *page = get_page(mm->pgdir, dst, NULL);
        assert(page != NULL);
        memcpy(page2kva(page) + PGOFF(dst), &zero, sizeof(uintptr_t));
    }
    ...
    sp -= sizeof(int);
    {
        ...
        struct Page *page = get_page(mm->pgdir, sp, NULL);
        assert(page != NULL);
        memcpy(page2kva(page) + PGOFF(sp), &cargc, sizeof(int));
    }

    // leave some breathing room below the argument block to avoid touching
    // just-below-USTACKTOP addresses in early user code prologues
    sp -= 0x80;
    sp &= ~((uintptr_t)0xF);
    ...
    // run in user mode: clear SPP, set SPIE to enable interrupts after sret
