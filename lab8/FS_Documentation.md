# Lab8 文件系统相关代码详解（可点击跳转）

本文件由脚本 `lab8/tools/gen_fs_doc.py` 自动生成：
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `main` | [badsegment.c#L6](user/badsegment.c#L6) | /* try to load the kernel's TSS selector into the DS register */ |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `cputch` | [stdio.c#L12](user/libs/stdio.c#L12) | /* *<br> * cputch - writes a single character @c to stdout, and it will<br> * increace the value of counter pointed by @cnt.<br> * */ |
| `vcprintf` | [stdio.c#L27](user/libs/stdio.c#L27) | /* *<br> * vcprintf - format a string and writes it to stdout<br> *<br> * The return value is the number of characters which would be<br> * written to stdout.<br> *<br> * Call this function if you are already dealing with a va_list.<br> * Or you probably want cprintf() instead.<br> * */ |
| `cprintf` | [stdio.c#L40](user/libs/stdio.c#L40) | /* *<br> * cprintf - formats a string and writes it to stdout<br> *<br> * The return value is the number of characters which would be<br> * written to stdout.<br> * */ |
| `cputs` | [stdio.c#L55](user/libs/stdio.c#L55) | /* *<br> * cputs- writes the string pointed by @str to stdout and<br> * appends a newline character.<br> * */ |
...
| `print_pgdir` | [ulib.c#L45](user/libs/ulib.c#L45) | //print_pgdir - print the PDT&PT |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `dev_open` | [dev.c#L12](kern/fs/devs/dev.c#L12) | /*<br> * dev_open - Called for each open().<br> */ |
| `dev_close` | [dev.c#L24](kern/fs/devs/dev.c#L24) | /*<br> * dev_close - Called on the last close(). Just pass through.<br> */ |
| `dev_read` | [dev.c#L33](kern/fs/devs/dev.c#L33) | /*<br> * dev_read -Called for read. Hand off to iobuf.<br> */ |
| `dev_write` | [dev.c#L42](kern/fs/devs/dev.c#L42) | /*<br> * dev_write -Called for write. Hand off to iobuf.<br> */ |
| `dev_ioctl` | [dev.c#L51](kern/fs/devs/dev.c#L51) | /*<br> * dev_ioctl - Called for ioctl(). Just pass through.<br> */ |
| `dev_fstat` | [dev.c#L62](kern/fs/devs/dev.c#L62) | /*<br> * dev_fstat - Called for stat().<br> *             Set the type and the size (block devices only).<br> *             The link count for a device is always 1.<br> */ |
| `dev_gettype` | [dev.c#L81](kern/fs/devs/dev.c#L81) | /*<br> * dev_gettype - Return the type. A device is a "block device" if it has a known<br> *               length. A device that generates data in a stream is a "character<br> *               device".<br> */ |
| `dev_tryseek` | [dev.c#L93](kern/fs/devs/dev.c#L93) | /*<br> * dev_tryseek - Attempt a seek.<br> *               For block devices, require block alignment.<br> *               For character devices, prohibit seeking entirely.<br> */ |
| `dev_lookup` | [dev.c#L118](kern/fs/devs/dev.c#L118) | /*<br> * dev_lookup - Name lookup.<br> *<br> * One interesting feature of device:name pathname syntax is that you<br> * can implement pathnames on arbitrary devices. For instance, if you<br> * had a graphics device that supported multiple resolutions (which we<br> * don't), you might arrange things so that you could open it with<br> * pathnames like "video:800x600/24bpp" in order to select the operating<br> * mode.<br> *<br> * However, we have no support for this in the base system.<br> */ |
| `dev_init` | [dev.c#L151](kern/fs/devs/dev.c#L151) | /* dev_init - Initialization functions for builtin vfs-level devices. */ |
| `dev_create_inode` | [dev.c#L159](kern/fs/devs/dev.c#L159) | /* dev_create_inode - Create inode for a vfs-level device. */ |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `get_fd_array` | [file.c#L17](kern/fs/file.c#L17) | // get_fd_array - get current process's open files table |
| `fd_array_init` | [file.c#L25](kern/fs/file.c#L25) | // fd_array_init - initialize the open files table |
| `fd_array_alloc` | [file.c#L36](kern/fs/file.c#L36) | // fs_array_alloc - allocate a free file item (with FD_NONE status) in open files table |
| `fd_array_free` | [file.c#L66](kern/fs/file.c#L66) | // fd_array_free - free a file item in open files table |
| `fd_array_acquire` | [file.c#L76](kern/fs/file.c#L76) | （建议直接点链接阅读实现细节；本函数缺少紧邻注释。） |
| `fd_array_release` | [file.c#L83](kern/fs/file.c#L83) | // fd_array_release - file's open_count--; if file's open_count-- == 0 , then call fd_array_free to free this file item |
| `fd_array_open` | [file.c#L93](kern/fs/file.c#L93) | // fd_array_open - file's open_count++, set status to FD_OPENED |
| `fd_array_close` | [file.c#L101](kern/fs/file.c#L101) | // fd_array_close - file's open_count--; if file's open_count-- == 0 , then call fd_array_free to free this file item |
| `fd_array_dup` | [file.c#L112](kern/fs/file.c#L112) | //fs_array_dup - duplicate file 'from'  to file 'to' |
| `fd2file` | [file.c#L126](kern/fs/file.c#L126) | // fd2file - use fd as index of fd_array, return the array item (file) |
| `file_testfd` | [file.c#L139](kern/fs/file.c#L139) | // file_testfd - test file is readble or writable? |
| `file_open` | [file.c#L156](kern/fs/file.c#L156) | // open file |
| `file_close` | [file.c#L196](kern/fs/file.c#L196) | // close file |
| `file_read` | [file.c#L208](kern/fs/file.c#L208) | // read file |
| `file_write` | [file.c#L234](kern/fs/file.c#L234) | // write file |
| `file_seek` | [file.c#L260](kern/fs/file.c#L260) | // seek file |
| `file_fstat` | [file.c#L292](kern/fs/file.c#L292) | // stat file |
| `file_fsync` | [file.c#L306](kern/fs/file.c#L306) | // sync file |
| `file_getdirentry` | [file.c#L320](kern/fs/file.c#L320) | // get file entry in DIR |
| `file_dup` | [file.c#L338](kern/fs/file.c#L338) | // duplicate file |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `fs_init` | [fs.c#L11](kern/fs/fs.c#L11) | //called when init_main proc start |
...
| `files_create` | [fs.c#L33](kern/fs/fs.c#L33) | //Called when a new proc init |
| `files_destroy` | [fs.c#L48](kern/fs/fs.c#L48) | //Called when a proc exit |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `bitmap_create` | [bitmap.c#L18](kern/fs/sfs/bitmap.c#L18) | // bitmap_create - allocate a new bitmap object. |
| `bitmap_alloc` | [bitmap.c#L53](kern/fs/sfs/bitmap.c#L53) | // bitmap_alloc - locate a cleared bit, set it, and return its index. |
| `bitmap_translate` | [bitmap.c#L74](kern/fs/sfs/bitmap.c#L74) | // bitmap_translate - according index, get the related word and mask |
| `bitmap_test` | [bitmap.c#L83](kern/fs/sfs/bitmap.c#L83) | // bitmap_test - according index, get the related value (0 OR 1) in the bitmap |
| `bitmap_free` | [bitmap.c#L91](kern/fs/sfs/bitmap.c#L91) | // bitmap_free - according index, set related bit to 1 |
| `bitmap_destroy` | [bitmap.c#L100](kern/fs/sfs/bitmap.c#L100) | // bitmap_destroy - free memory contains bitmap |
| `bitmap_getdata` | [bitmap.c#L107](kern/fs/sfs/bitmap.c#L107) | // bitmap_getdata - return bitmap->map, return the length of bits to len_store |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `sfs_init` | [sfs.c#L12](kern/fs/sfs/sfs.c#L12) | /*<br> * sfs_init - mount sfs on disk0<br> *<br> * CALL GRAPH:<br> *   kern_init-->fs_init-->sfs_init<br> */ |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `sfs_sync` | [sfs_fs.c#L19](kern/fs/sfs/sfs_fs.c#L19) | /*<br> * sfs_sync - sync sfs's superblock and freemap in memroy into disk<br> */ |
| `sfs_get_root` | [sfs_fs.c#L50](kern/fs/sfs/sfs_fs.c#L50) | /*<br> * sfs_get_root - get the root directory inode  from disk (SFS_BLKN_ROOT,1)<br> */ |
| `sfs_unmount` | [sfs_fs.c#L63](kern/fs/sfs/sfs_fs.c#L63) | /*<br> * sfs_unmount - unmount sfs, and free the memorys contain sfs->freemap/sfs_buffer/hash_liskt and sfs itself.<br> */ |
| `sfs_cleanup` | [sfs_fs.c#L82](kern/fs/sfs/sfs_fs.c#L82) | /*<br> * sfs_cleanup - when sfs failed, then should call this function to sync sfs by calling sfs_sync<br> *<br> * NOTICE: nouse now.<br> */ |
| `sfs_init_read` | [sfs_fs.c#L109](kern/fs/sfs/sfs_fs.c#L109) | /*<br> * sfs_init_read - used in sfs_do_mount to read disk block(blkno, 1) directly.<br> *<br> * @dev:        the block device<br> * @blkno:      the NO. of disk block<br> * @blk_buffer: the buffer used for read<br> *<br> *      (1) init iobuf<br> *      (2) read dev into iobuf<br> */ |
| `sfs_init_freemap` | [sfs_fs.c#L127](kern/fs/sfs/sfs_fs.c#L127) | /*<br> * sfs_init_freemap - used in sfs_do_mount to read freemap data info in disk block(blkno, nblks) directly.<br> *<br> * @dev:        the block device<br> * @bitmap:     the bitmap in memroy<br> * @blkno:      the NO. of disk block<br> * @nblks:      Rd number of disk block<br> * @blk_buffer: the buffer used for read<br> *<br> *      (1) get data addr in bitmap<br> *      (2) read dev into iobuf<br> */ |
| `sfs_do_mount` | [sfs_fs.c#L148](kern/fs/sfs/sfs_fs.c#L148) | /*<br> * sfs_do_mount - mount sfs file system.<br> *<br> * @dev:        the block device contains sfs file system<br> * @fs_store:   the fs struct in memroy<br> */ |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `lock_sin` | [sfs_inode.c#L22](kern/fs/sfs/sfs_inode.c#L22) | /*<br> * lock_sin - lock the process of inode Rd/Wr<br> */ |
| `unlock_sin` | [sfs_inode.c#L30](kern/fs/sfs/sfs_inode.c#L30) | /*<br> * unlock_sin - unlock the process of inode Rd/Wr<br> */ |
| `sfs_get_ops` | [sfs_inode.c#L38](kern/fs/sfs/sfs_inode.c#L38) | /*<br> * sfs_get_ops - return function addr of fs_node_dirops/sfs_node_fileops<br> */ |
| `sfs_hash_list` | [sfs_inode.c#L52](kern/fs/sfs/sfs_inode.c#L52) | /*<br> * sfs_hash_list - return inode entry in sfs->hash_list<br> */ |
| `sfs_set_links` | [sfs_inode.c#L60](kern/fs/sfs/sfs_inode.c#L60) | /*<br> * sfs_set_links - link inode sin in sfs->linked-list AND sfs->hash_link<br> */ |
| `sfs_remove_links` | [sfs_inode.c#L69](kern/fs/sfs/sfs_inode.c#L69) | /*<br> * sfs_remove_links - unlink inode sin in sfs->linked-list AND sfs->hash_link<br> */ |
| `sfs_block_inuse` | [sfs_inode.c#L78](kern/fs/sfs/sfs_inode.c#L78) | /*<br> * sfs_block_inuse - check the inode with NO. ino inuse info in bitmap<br> */ |
| `sfs_block_alloc` | [sfs_inode.c#L89](kern/fs/sfs/sfs_inode.c#L89) | /*<br> * sfs_block_alloc -  check and get a free disk block<br> */ |
| `sfs_block_free` | [sfs_inode.c#L104](kern/fs/sfs/sfs_inode.c#L104) | /*<br> * sfs_block_free - set related bits for ino block to 1(means free) in bitmap, add sfs->super.unused_blocks, set superblock dirty *<br> */ |
| `sfs_create_inode` | [sfs_inode.c#L114](kern/fs/sfs/sfs_inode.c#L114) | /*<br> * sfs_create_inode - alloc a inode in memroy, and init din/ino/dirty/reclian_count/sem fields in sfs_inode in inode<br> */ |
| `lookup_sfs_nolock` | [sfs_inode.c#L133](kern/fs/sfs/sfs_inode.c#L133) | /*<br> * lookup_sfs_nolock - according ino, find related inode<br> *<br> * NOTICE: le2sin, info2node MACRO<br> */ |
| `sfs_load_inode` | [sfs_inode.c#L154](kern/fs/sfs/sfs_inode.c#L154) | /*<br> * sfs_load_inode - If the inode isn't existed, load inode related ino disk block data into a new created inode.<br> *                  If the inode is in memory alreadily, then do nothing<br> */ |
| `sfs_bmap_get_sub_nolock` | [sfs_inode.c#L200](kern/fs/sfs/sfs_inode.c#L200) | /*<br> * sfs_bmap_get_sub_nolock - according entry pointer entp and index, find the index of indrect disk block<br> *                           return the index of indrect disk block to ino_store. no lock protect<br> * @sfs:      sfs file system<br> * @entp:     the pointer of index of entry disk block<br> * @index:    the index of block in indrect block<br> * @create:   BOOL, if the block isn't allocated, if create = 1 the alloc a block,  otherwise just do nothing<br> * @ino_store: 0 OR the index of already inused block or new allocated block.<br> */ |
...
| `sfs_bmap_free_sub_nolock` | [sfs_inode.c#L297](kern/fs/sfs/sfs_inode.c#L297) | /*<br> * sfs_bmap_free_sub_nolock - set the entry item to 0 (free) in the indirect block<br> */ |
| `sfs_bmap_free_nolock` | [sfs_inode.c#L318](kern/fs/sfs/sfs_inode.c#L318) | /*<br> * sfs_bmap_free_nolock - free a block with logical index in inode and reset the inode's fields<br> */ |
| `sfs_bmap_load_nolock` | [sfs_inode.c#L353](kern/fs/sfs/sfs_inode.c#L353) | /*<br> * sfs_bmap_load_nolock - according to the DIR's inode and the logical index of block in inode, find the NO. of disk block.<br> * @sfs:      sfs file system<br> * @sin:      sfs inode in memory<br> * @index:    the logical index of disk block in inode<br> * @ino_store:the NO. of disk block<br> */ |
| `sfs_bmap_truncate_nolock` | [sfs_inode.c#L376](kern/fs/sfs/sfs_inode.c#L376) | /*<br> * sfs_bmap_truncate_nolock - free the disk block at the end of file<br> */ |
| `sfs_dirent_read_nolock` | [sfs_inode.c#L396](kern/fs/sfs/sfs_inode.c#L396) | /*<br> * sfs_dirent_read_nolock - read the file entry from disk block which contains this entry<br> * @sfs:      sfs file system<br> * @sin:      sfs inode in memory<br> * @slot:     the index of file entry<br> * @entry:    file entry<br> */ |
| `sfs_dirent_search_nolock` | [sfs_inode.c#L440](kern/fs/sfs/sfs_inode.c#L440) | /*<br> * sfs_dirent_search_nolock - read every file entry in the DIR, compare file name with each entry->name<br> *                            If equal, then return slot and NO. of disk of this file's inode<br> * @sfs:        sfs file system<br> * @sin:        sfs inode in memory<br> * @name:       the filename<br> * @ino_store:  NO. of disk of this file (with the filename)'s inode<br> * @slot:       logical index of file entry (NOTICE: each file entry ocupied one  disk block)<br> * @empty_slot: the empty logical index of file entry.<br> */ |
| `sfs_dirent_findino_nolock` | [sfs_inode.c#L476](kern/fs/sfs/sfs_inode.c#L476) | /*<br> * sfs_dirent_findino_nolock - read all file entries in DIR's inode and find a entry->ino == ino<br> */ |
| `sfs_lookup_once` | [sfs_inode.c#L498](kern/fs/sfs/sfs_inode.c#L498) | /*<br> * sfs_lookup_once - find inode corresponding the file name in DIR's sin inode <br> * @sfs:        sfs file system<br> * @sin:        DIR sfs inode in memory<br> * @name:       the file name in DIR<br> * @node_store: the inode corresponding the file name in DIR<br> * @slot:       the logical index of file entry<br> */ |
| `sfs_opendir` | [sfs_inode.c#L515](kern/fs/sfs/sfs_inode.c#L515) | // sfs_opendir - just check the opne_flags, now support readonly |
| `sfs_openfile` | [sfs_inode.c#L532](kern/fs/sfs/sfs_inode.c#L532) | // sfs_openfile - open file (no use) |
| `sfs_close` | [sfs_inode.c#L538](kern/fs/sfs/sfs_inode.c#L538) | // sfs_close - close file |
...
| `sfs_io` | [sfs_inode.c#L666](kern/fs/sfs/sfs_inode.c#L666) | /*<br> * sfs_io - Rd/Wr file. the wrapper of sfs_io_nolock<br>            with lock protect<br> */ |
| `sfs_read` | [sfs_inode.c#L684](kern/fs/sfs/sfs_inode.c#L684) | // sfs_read - read file |
| `sfs_write` | [sfs_inode.c#L690](kern/fs/sfs/sfs_inode.c#L690) | // sfs_write - write file |
| `sfs_fstat` | [sfs_inode.c#L698](kern/fs/sfs/sfs_inode.c#L698) | /*<br> * sfs_fstat - Return nlinks/block/size, etc. info about a file. The pointer is a pointer to struct stat;<br> */ |
| `sfs_fsync` | [sfs_inode.c#L715](kern/fs/sfs/sfs_inode.c#L715) | /*<br> * sfs_fsync - Force any dirty inode info associated with this file to stable storage.<br> */ |
| `sfs_namefile` | [sfs_inode.c#L739](kern/fs/sfs/sfs_inode.c#L739) | /*<br> *sfs_namefile -Compute pathname relative to filesystem root of the file and copy to the specified io buffer.<br> *  <br> */ |
| `sfs_getdirentry_sub_nolock` | [sfs_inode.c#L804](kern/fs/sfs/sfs_inode.c#L804) | /*<br> * sfs_getdirentry_sub_noblock - get the content of file entry in DIR<br> */ |
| `sfs_getdirentry` | [sfs_inode.c#L825](kern/fs/sfs/sfs_inode.c#L825) | /*<br> * sfs_getdirentry - according to the iob->io_offset, calculate the dir entry's slot in disk block,<br>                     get dir entry content from the disk <br> */ |
| `sfs_reclaim` | [sfs_inode.c#L860](kern/fs/sfs/sfs_inode.c#L860) | /*<br> * sfs_reclaim - Free all resources inode occupied . Called when inode is no longer in use. <br> */ |
| `sfs_gettype` | [sfs_inode.c#L903](kern/fs/sfs/sfs_inode.c#L903) | /*<br> * sfs_gettype - Return type of file. The values for file types are in sfs.h.<br> */ |
| `sfs_tryseek` | [sfs_inode.c#L923](kern/fs/sfs/sfs_inode.c#L923) | /* <br> * sfs_tryseek - Check if seeking to the specified position within the file is legal.<br> */ |
| `sfs_truncfile` | [sfs_inode.c#L938](kern/fs/sfs/sfs_inode.c#L938) | /*<br> * sfs_truncfile : reszie the file with new length<br> */ |
| `sfs_lookup` | [sfs_inode.c#L990](kern/fs/sfs/sfs_inode.c#L990) | /*<br> * sfs_lookup - Parse path relative to the passed directory<br> *              DIR, and hand back the inode for the file it<br> *              refers to.<br> */ |
...
| `sfs_rwblock` | [sfs_io.c#L34](kern/fs/sfs/sfs_io.c#L34) | /* sfs_rwblock - Basic block-level I/O routine for Rd/Wr N disk blocks ,<br> *               with lock protect for mutex process on Rd/Wr disk block<br> * @sfs:   sfs_fs which will be process<br> * @buf:   the buffer uesed for Rd/Wr<br> * @blkno: the NO. of disk block<br> * @nblks: Rd/Wr number of disk block<br> * @write: BOOL: Read - 0 or Write - 1<br> */ |
| `sfs_rblock` | [sfs_io.c#L58](kern/fs/sfs/sfs_io.c#L58) | /* sfs_rblock - The Wrap of sfs_rwblock function for Rd N disk blocks ,<br> *<br> * @sfs:   sfs_fs which will be process<br> * @buf:   the buffer uesed for Rd/Wr<br> * @blkno: the NO. of disk block<br> * @nblks: Rd/Wr number of disk block<br> */ |
| `sfs_wblock` | [sfs_io.c#L70](kern/fs/sfs/sfs_io.c#L70) | /* sfs_wblock - The Wrap of sfs_rwblock function for Wr N disk blocks ,<br> *<br> * @sfs:   sfs_fs which will be process<br> * @buf:   the buffer uesed for Rd/Wr<br> * @blkno: the NO. of disk block<br> * @nblks: Rd/Wr number of disk block<br> */ |
| `sfs_rbuf` | [sfs_io.c#L83](kern/fs/sfs/sfs_io.c#L83) | /* sfs_rbuf - The Basic block-level I/O routine for  Rd( non-block & non-aligned io) one disk block(using sfs->sfs_buffer)<br> *            with lock protect for mutex process on Rd/Wr disk block<br> * @sfs:    sfs_fs which will be process<br> * @buf:    the buffer uesed for Rd<br> * @len:    the length need to Rd<br> * @blkno:  the NO. of disk block<br> * @offset: the offset in the content of disk block<br> */ |
| `sfs_wbuf` | [sfs_io.c#L105](kern/fs/sfs/sfs_io.c#L105) | /* sfs_wbuf - The Basic block-level I/O routine for  Wr( non-block & non-aligned io) one disk block(using sfs->sfs_buffer)<br> *            with lock protect for mutex process on Rd/Wr disk block<br> * @sfs:    sfs_fs which will be process<br> * @buf:    the buffer uesed for Wr<br> * @len:    the length need to Wr<br> * @blkno:  the NO. of disk block<br> * @offset: the offset in the content of disk block<br> */ |
| `sfs_sync_super` | [sfs_io.c#L123](kern/fs/sfs/sfs_io.c#L123) | /*<br> * sfs_sync_super - write sfs->super (in memory) into disk (SFS_BLKN_SUPER, 1) with lock protect.<br> */ |
| `sfs_sync_freemap` | [sfs_io.c#L139](kern/fs/sfs/sfs_io.c#L139) | /*<br> * sfs_sync_freemap - write sfs bitmap into disk (SFS_BLKN_FREEMAP, nblks)  without lock protect.<br> */ |
| `sfs_clear_block` | [sfs_io.c#L151](kern/fs/sfs/sfs_io.c#L151) | /*<br> * sfs_clear_block - write zero info into disk (blkno, nblks)  with lock protect.<br> * @sfs:   sfs_fs which will be process<br> * @blkno: the NO. of disk block<br> * @nblks: Rd/Wr number of disk block<br> */ |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `lock_sfs_fs` | [sfs_lock.c#L11](kern/fs/sfs/sfs_lock.c#L11) | /*<br> * lock_sfs_fs - lock the process of  SFS Filesystem Rd/Wr Disk Block<br> *<br> * called by: sfs_load_inode, sfs_sync, sfs_reclaim<br> */ |
| `lock_sfs_io` | [sfs_lock.c#L21](kern/fs/sfs/sfs_lock.c#L21) | /*<br> * lock_sfs_io - lock the process of SFS File Rd/Wr Disk Block<br> *<br> * called by: sfs_rwblock, sfs_clear_block, sfs_sync_super<br> */ |
| `unlock_sfs_fs` | [sfs_lock.c#L31](kern/fs/sfs/sfs_lock.c#L31) | /*<br> * unlock_sfs_fs - unlock the process of  SFS Filesystem Rd/Wr Disk Block<br> *<br> * called by: sfs_load_inode, sfs_sync, sfs_reclaim<br> */ |
| `unlock_sfs_io` | [sfs_lock.c#L41](kern/fs/sfs/sfs_lock.c#L41) | /*<br> * unlock_sfs_io - unlock the process of sfs Rd/Wr Disk Block<br> *<br> * called by: sfs_rwblock sfs_clear_block sfs_sync_super<br> */ |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `copy_path` | [sysfile.c#L19](kern/fs/sysfile.c#L19) | /* copy_path - copy path name */ |
| `sysfile_open` | [sysfile.c#L41](kern/fs/sysfile.c#L41) | /* sysfile_open - open file */ |
| `sysfile_close` | [sysfile.c#L54](kern/fs/sysfile.c#L54) | /* sysfile_close - close file */ |
| `sysfile_read` | [sysfile.c#L60](kern/fs/sysfile.c#L60) | /* sysfile_read - read file */ |
| `sysfile_write` | [sysfile.c#L108](kern/fs/sysfile.c#L108) | /* sysfile_write - write file */ |
| `sysfile_seek` | [sysfile.c#L156](kern/fs/sysfile.c#L156) | /* sysfile_seek - seek file */ |
| `sysfile_fstat` | [sysfile.c#L162](kern/fs/sysfile.c#L162) | /* sysfile_fstat - stat file */ |
| `sysfile_fsync` | [sysfile.c#L182](kern/fs/sysfile.c#L182) | /* sysfile_fsync - sync file */ |
| `sysfile_chdir` | [sysfile.c#L188](kern/fs/sysfile.c#L188) | /* sysfile_chdir - change dir */ |
| `sysfile_link` | [sysfile.c#L201](kern/fs/sysfile.c#L201) | /* sysfile_link - link file */ |
| `sysfile_rename` | [sysfile.c#L218](kern/fs/sysfile.c#L218) | /* sysfile_rename - rename file */ |
| `sysfile_unlink` | [sysfile.c#L235](kern/fs/sysfile.c#L235) | /* sysfile_unlink - unlink file */ |
| `sysfile_getcwd` | [sysfile.c#L248](kern/fs/sysfile.c#L248) | /* sysfile_get cwd - get current working directory */ |
| `sysfile_getdirentry` | [sysfile.c#L268](kern/fs/sysfile.c#L268) | /* sysfile_getdirentry - get the file entry in DIR */ |
| `sysfile_dup` | [sysfile.c#L303](kern/fs/sysfile.c#L303) | /* sysfile_dup -  duplicate fd1 to fd2 */ |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `__alloc_inode` | [inode.c#L14](kern/fs/vfs/inode.c#L14) | /* *<br> * __alloc_inode - alloc a inode structure and initialize in_type<br> * */ |
| `inode_init` | [inode.c#L27](kern/fs/vfs/inode.c#L27) | /* *<br> * inode_init - initialize a inode structure<br> * invoked by vop_init<br> * */ |
| `inode_kill` | [inode.c#L39](kern/fs/vfs/inode.c#L39) | /* *<br> * inode_kill - kill a inode structure<br> * invoked by vop_kill<br> * */ |
| `inode_ref_inc` | [inode.c#L50](kern/fs/vfs/inode.c#L50) | /* *<br> * inode_ref_inc - increment ref_count<br> * invoked by vop_ref_inc<br> * */ |
| `inode_ref_dec` | [inode.c#L61](kern/fs/vfs/inode.c#L61) | /* *<br> * inode_ref_dec - decrement ref_count<br> * invoked by vop_ref_dec<br> * calls vop_reclaim if the ref_count hits zero<br> * */ |
| `inode_open_inc` | [inode.c#L79](kern/fs/vfs/inode.c#L79) | /* *<br> * inode_open_inc - increment the open_count<br> * invoked by vop_open_inc<br> * */ |
| `inode_open_dec` | [inode.c#L90](kern/fs/vfs/inode.c#L90) | /* *<br> * inode_open_dec - decrement the open_count<br> * invoked by vop_open_dec<br> * calls vop_close if the open_count hits zero<br> * */ |
| `inode_check` | [inode.c#L108](kern/fs/vfs/inode.c#L108) | /* *<br> * inode_check - check the various things being valid<br> * called before all vop_* calls<br> * */ |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `__alloc_fs` | [vfs.c#L16](kern/fs/vfs/vfs.c#L16) | // __alloc_fs - allocate memory for fs, and set fs type |
| `vfs_init` | [vfs.c#L26](kern/fs/vfs/vfs.c#L26) | // vfs_init -  vfs initialize |
| `lock_bootfs` | [vfs.c#L33](kern/fs/vfs/vfs.c#L33) | // lock_bootfs - lock  for bootfs |
| `unlock_bootfs` | [vfs.c#L38](kern/fs/vfs/vfs.c#L38) | // ulock_bootfs - ulock for bootfs |
| `change_bootfs` | [vfs.c#L44](kern/fs/vfs/vfs.c#L44) | // change_bootfs - set the new fs inode |
| `vfs_set_bootfs` | [vfs.c#L58](kern/fs/vfs/vfs.c#L58) | // vfs_set_bootfs - change the dir of file system |
| `vfs_get_bootfs` | [vfs.c#L79](kern/fs/vfs/vfs.c#L79) | // vfs_get_bootfs - get the inode of bootfs |
...
| `vfs_cleanup` | [vfsdev.c#L46](kern/fs/vfs/vfsdev.c#L46) | // vfs_cleanup - finally clean (or sync) fs |
| `vfs_get_root` | [vfsdev.c#L67](kern/fs/vfs/vfsdev.c#L67) | /*<br> * vfs_get_root - Given a device name (stdin, stdout, etc.), hand<br> *                back an appropriate inode.<br> */ |
| `vfs_get_devname` | [vfsdev.c#L104](kern/fs/vfs/vfsdev.c#L104) | /*<br> * vfs_get_devname - Given a filesystem, hand back the name of the device it's mounted on.<br> */ |
| `check_devname_conflict` | [vfsdev.c#L120](kern/fs/vfs/vfsdev.c#L120) | /*<br> * check_devname_confilct - Is there alreadily device which has the same name?<br> */ |
| `vfs_do_add` | [vfsdev.c#L140](kern/fs/vfs/vfsdev.c#L140) | /*<br>* vfs_do_add - Add a new device to the VFS layer's device table.<br>*<br>* If "mountable" is set, the device will be treated as one that expects<br>* to have a filesystem mounted on it, and a raw device will be created<br>* for direct access.<br>*/ |
| `vfs_add_fs` | [vfsdev.c#L185](kern/fs/vfs/vfsdev.c#L185) | /*<br> * vfs_add_fs - Add a new fs,  by name. See  vfs_do_add information for the description of<br> *              mountable.<br> */ |
| `vfs_add_dev` | [vfsdev.c#L194](kern/fs/vfs/vfsdev.c#L194) | /*<br> * vfs_add_dev - Add a new device, by name. See  vfs_do_add information for the description of<br> *               mountable.<br> */ |
| `find_mount` | [vfsdev.c#L203](kern/fs/vfs/vfsdev.c#L203) | /*<br> * find_mount - Look for a mountable device named DEVNAME.<br> *              Should already hold vdev_list lock.<br> */ |
| `int` | [vfsdev.c#L223](kern/fs/vfs/vfsdev.c#L223) | /*<br> * vfs_mount - Mount a filesystem. Once we've found the device, call MOUNTFUNC to<br> *             set up the filesystem and hand back a struct fs.<br> *<br> * The DATA argument is passed through unchanged to MOUNTFUNC.<br> */ |
| `vfs_unmount` | [vfsdev.c#L252](kern/fs/vfs/vfsdev.c#L252) | /*<br> * vfs_unmount - Unmount a filesystem/device by name.<br> *               First calls FSOP_SYNC on the filesystem; then calls FSOP_UNMOUNT.<br> */ |
| `vfs_unmount_all` | [vfsdev.c#L282](kern/fs/vfs/vfsdev.c#L282) | /*<br> * vfs_unmount_all - Global unmount function.<br> */ |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `vfs_open` | [vfsfile.c#L11](kern/fs/vfs/vfsfile.c#L11) | // open file in vfs, get/create inode for file with filename path. |
| `vfs_close` | [vfsfile.c#L69](kern/fs/vfs/vfsfile.c#L69) | // close file in vfs |
| `vfs_unlink` | [vfsfile.c#L77](kern/fs/vfs/vfsfile.c#L77) | // unimplement |
| `vfs_rename` | [vfsfile.c#L83](kern/fs/vfs/vfsfile.c#L83) | // unimplement |
| `vfs_link` | [vfsfile.c#L89](kern/fs/vfs/vfsfile.c#L89) | // unimplement |
| `vfs_symlink` | [vfsfile.c#L95](kern/fs/vfs/vfsfile.c#L95) | // unimplement |
| `vfs_readlink` | [vfsfile.c#L101](kern/fs/vfs/vfsfile.c#L101) | // unimplement |
| `vfs_mkdir` | [vfsfile.c#L107](kern/fs/vfs/vfsfile.c#L107) | // unimplement |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `get_device` | [vfslookup.c#L13](kern/fs/vfs/vfslookup.c#L13) | /*<br> * get_device- Common code to pull the device name, if any, off the front of a<br> *             path and choose the inode to begin the name lookup relative to.<br> */ |
| `vfs_lookup` | [vfslookup.c#L71](kern/fs/vfs/vfslookup.c#L71) | /*<br> * vfs_lookup - get the inode according to the path filename<br> */ |
| `vfs_lookup_parent` | [vfslookup.c#L91](kern/fs/vfs/vfslookup.c#L91) | /*<br> * vfs_lookup_parent - Name-to-vnode translation.<br> *  (In BSD, both of these are subsumed by namei().)<br> */ |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `get_cwd_nolock` | [vfspath.c#L14](kern/fs/vfs/vfspath.c#L14) | /*<br> * get_cwd_nolock - retrieve current process's working directory. without lock protect<br> */ |
| `set_cwd_nolock` | [vfspath.c#L21](kern/fs/vfs/vfspath.c#L21) | /*<br> * set_cwd_nolock - set current working directory.<br> */ |
| `lock_cfs` | [vfspath.c#L29](kern/fs/vfs/vfspath.c#L29) | /*<br> * lock_cfs - lock the fs related process on current process <br> */ |
| `unlock_cfs` | [vfspath.c#L36](kern/fs/vfs/vfspath.c#L36) | /*<br> * unlock_cfs - unlock the fs related process on current process <br> */ |
| `vfs_get_curdir` | [vfspath.c#L44](kern/fs/vfs/vfspath.c#L44) | /*<br> *  vfs_get_curdir - Get current directory as a inode.<br> */ |
| `vfs_set_curdir` | [vfspath.c#L59](kern/fs/vfs/vfspath.c#L59) | /*<br> * vfs_set_curdir - Set current directory as a inode.<br> *                  The passed inode must in fact be a directory.<br> */ |
| `vfs_chdir` | [vfspath.c#L90](kern/fs/vfs/vfspath.c#L90) | /*<br> * vfs_chdir - Set current directory, as a pathname. Use vfs_lookup to translate<br> *             it to a inode.<br> */ |
| `vfs_getcwd` | [vfspath.c#L103](kern/fs/vfs/vfspath.c#L103) | /*<br> * vfs_getcwd - retrieve current working directory(cwd).<br> */ |
...
| 函数 | 源码跳转 | 说明（原注释/逻辑要点） |
|---|---:|---|
| `alloc_proc` | [proc.c#L88](kern/process/proc.c#L88) | // alloc_proc - alloc a proc_struct and init all fields of proc_struct |
| `set_proc_name` | [proc.c#L165](kern/process/proc.c#L165) | // set_proc_name - set the name of proc |
| `get_proc_name` | [proc.c#L173](kern/process/proc.c#L173) | // get_proc_name - get the name of proc |
| `set_links` | [proc.c#L182](kern/process/proc.c#L182) | // set_links - set the relation links of process |
| `remove_links` | [proc.c#L196](kern/process/proc.c#L196) | // remove_links - clean the relation links of process |
| `get_pid` | [proc.c#L216](kern/process/proc.c#L216) | // get_pid - alloc a unique pid for process |
| `proc_run` | [proc.c#L260](kern/process/proc.c#L260) | // proc_run - make process "proc" running on cpu<br>// NOTE: before call switch_to, should load  base addr of "proc"'s new PDT |
| `forkret` | [proc.c#L296](kern/process/proc.c#L296) | // forkret -- the first kernel entry point of a new thread/process<br>// NOTE: the addr of forkret is setted in copy_thread function<br>//       after switch_to, the current proc will execute here. |
| `hash_proc` | [proc.c#L303](kern/process/proc.c#L303) | // hash_proc - add proc into proc hash_list |
| `unhash_proc` | [proc.c#L310](kern/process/proc.c#L310) | // unhash_proc - delete proc from proc hash_list |
| `find_proc` | [proc.c#L317](kern/process/proc.c#L317) | // find_proc - find proc frome proc hash_list according to pid |
| `int` | [proc.c#L338](kern/process/proc.c#L338) | // kernel_thread - create a kernel thread using "fn" function<br>// NOTE: the contents of temp trapframe tf will be copied to<br>//       proc->tf in do_fork-->copy_thread function |
| `setup_kstack` | [proc.c#L350](kern/process/proc.c#L350) | // setup_kstack - alloc pages with size KSTACKPAGE as process kernel stack |
| `put_kstack` | [proc.c#L363](kern/process/proc.c#L363) | // put_kstack - free the memory space of process kernel stack |
| `setup_pgdir` | [proc.c#L370](kern/process/proc.c#L370) | // setup_pgdir - alloc one page as PDT |
| `put_pgdir` | [proc.c#L386](kern/process/proc.c#L386) | // put_pgdir - free the memory space of PDT |
| `copy_mm` | [proc.c#L394](kern/process/proc.c#L394) | // copy_mm - process "proc" duplicate OR share process "current"'s mm according clone_flags<br>//         - if clone_flags & CLONE_VM, then "share" ; else "duplicate" |
| `copy_thread` | [proc.c#L445](kern/process/proc.c#L445) | // copy_thread - setup the trapframe on the  process's kernel stack top and<br>//             - setup the kernel entry point and stack of process |
| `copy_files` | [proc.c#L460](kern/process/proc.c#L460) | // copy_files&put_files function used by do_fork in LAB8<br>// copy the files_struct from current to proc |
| `put_files` | [proc.c#L495](kern/process/proc.c#L495) | // decrease the ref_count of files, and if ref_count==0, then destroy files_struct |
| `do_fork` | [proc.c#L513](kern/process/proc.c#L513) | /* do_fork -     parent process for a new child process<br> * @clone_flags: used to guide how to clone the child process<br> * @stack:       the parent's user stack pointer. if stack==0, It means to fork a kernel thread.<br> * @tf:          the trapframe info, which will be copied to child process's proc->tf<br> */ |
| `do_exit` | [proc.c#L587](kern/process/proc.c#L587) | // do_exit - called by sys_exit<br>//   1. call exit_mmap & put_pgdir & mm_destroy to free the almost all memory space of process<br>//   2. set process' state as PROC_ZOMBIE, then call wakeup_proc(parent) to ask parent reclaim itself.<br>//   3. call scheduler to switch to other process |
| `load_icode_read` | [proc.c#L648](kern/process/proc.c#L648) | // load_icode_read is used by load_icode in LAB8 |
| `load_icode` | [proc.c#L665](kern/process/proc.c#L665) | // load_icode -  called by sys_exec-->do_execve |
| `put_kargv` | [proc.c#L1013](kern/process/proc.c#L1013) | // this function isn't very correct in LAB8 |
| `copy_kargv` | [proc.c#L1022](kern/process/proc.c#L1022) | （建议直接点链接阅读实现细节；本函数缺少紧邻注释。） |
| `do_execve` | [proc.c#L1055](kern/process/proc.c#L1055) | // do_execve - call exit_mmap(mm)&put_pgdir(mm) to reclaim memory space of current process<br>//           - call load_icode to setup new memory space accroding binary prog. |
| `do_yield` | [proc.c#L1128](kern/process/proc.c#L1128) | // do_yield - ask the scheduler to reschedule |
| `do_wait` | [proc.c#L1137](kern/process/proc.c#L1137) | // do_wait - wait one OR any children with PROC_ZOMBIE state, and free memory space of kernel stack<br>//         - proc struct of this child.<br>// NOTE: only after do_wait function, all resources of the child proces are free. |
| `do_kill` | [proc.c#L1209](kern/process/proc.c#L1209) | // do_kill - kill process with pid by set this process's flags with PF_EXITING |
| `kernel_execve` | [proc.c#L1229](kern/process/proc.c#L1229) | // kernel_execve - build a new trapframe, execute do_execve in-kernel, and return to user mode via __trapret |
| `user_main` | [proc.c#L1267](kern/process/proc.c#L1267) | // user_main - kernel thread used to exec a user program |
| `init_main` | [proc.c#L1283](kern/process/proc.c#L1283) | // init_main - the second kernel thread used to create user_main kernel threads |
| `proc_init` | [proc.c#L1321](kern/process/proc.c#L1321) | // proc_init - set up the first kernel thread idleproc "idle" by itself and<br>//           - create the second kernel thread init_main |
| `cpu_idle` | [proc.c#L1366](kern/process/proc.c#L1366) | // cpu_idle - at the end of kern_init, the first kernel thread idleproc will do below works |
| `lab6_set_priority` | [proc.c#L1377](kern/process/proc.c#L1377) | // FOR LAB6, set the process's priority (bigger value will get more CPU time) |
| `do_sleep` | [proc.c#L1387](kern/process/proc.c#L1387) | // do_sleep - set current process state to sleep and add timer with "time"<br>//          - then call scheduler. if process run again, delete timer first. |
...
- [put_kargv](kern/process/proc.c#L1013)

```c
static void put_kargv(int argc, char **kargv)
```

- [copy_kargv](kern/process/proc.c#L1022)

```c
static int copy_kargv(struct mm_struct *mm, int argc, char **kargv, const char **argv)
```

- [do_execve](kern/process/proc.c#L1055)

```c
int do_execve(const char *name, int argc, const char **argv)
```

- [do_yield](kern/process/proc.c#L1128)

```c
int do_yield(void)
```

- [do_wait](kern/process/proc.c#L1137)

```c
int do_wait(int pid, int *code_store)
```

- [do_kill](kern/process/proc.c#L1209)

```c
int do_kill(int pid)
```

- [kernel_execve](kern/process/proc.c#L1229)

```c
static int kernel_execve(const char *name, const char **argv)
```

- [user_main](kern/process/proc.c#L1267)

```c
static int user_main(void *arg)
```

- [init_main](kern/process/proc.c#L1283)

```c
static int init_main(void *arg)
```

- [proc_init](kern/process/proc.c#L1321)

```c
void proc_init(void)
```

- [cpu_idle](kern/process/proc.c#L1366)

```c
void cpu_idle(void)
```

- [lab6_set_priority](kern/process/proc.c#L1377)

```c
void lab6_set_priority(uint32_t priority)
```

- [do_sleep](kern/process/proc.c#L1387)
