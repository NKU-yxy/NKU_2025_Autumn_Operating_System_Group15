...
obj/__user_bench_short.out:     file format elf64-littleriscv
...
exit(int error_code) {
  ...
  8000da:	4f250513          	addi	a0,a0,1266 # 8005c8 <main+0xc6>
...
printnum(void (*putch)(int, void*), void *putdat,
  ...
  800136:	4ae78793          	addi	a5,a5,1198 # 8005e0 <main+0xde>
...
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap) {
  ...
  800192:	486a8a93          	addi	s5,s5,1158 # 800614 <main+0x112>
        case 'e':
            err = va_arg(ap, int);
            if (err < 0) {
                err = -err;
  ...
  80019a:	69ab8b93          	addi	s7,s7,1690 # 800830 <error_string>
        while ((ch = *(unsigned char *)fmt ++) != '%') {
  ...
  8003f0:	22460613          	addi	a2,a2,548 # 800610 <main+0x10e>
  ...
  800402:	20260613          	addi	a2,a2,514 # 800600 <main+0xfe>
  ...
  800414:	1e840413          	addi	s0,s0,488 # 8005f8 <main+0xf6>
  ...
  80049e:	15e40413          	addi	s0,s0,350 # 8005f8 <main+0xf6>
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  ...
  8004b4:	149a0a13          	addi	s4,s4,329 # 8005f9 <main+0xf7>
...
int main(void) {
  800502:	7139                	addi	sp,sp,-64
  800504:	f822                	sd	s0,48(sp)
  800506:	0020                	addi	s0,sp,8
  800508:	f426                	sd	s1,40(sp)
  80050a:	f04a                	sd	s2,32(sp)
  80050c:	fc06                	sd	ra,56(sp)
  80050e:	01c10913          	addi	s2,sp,28
  800512:	84a2                	mv	s1,s0
    int i;
    int pids[NCHILD];

    for (i = 0; i < NCHILD; i++) {
        int pid = fork();
  800514:	bd1ff0ef          	jal	ra,8000e4 <fork>
        if (pid == 0) {
  800518:	c915                	beqz	a0,80054c <main+0x4a>
            }
        ...
        pids[i] = pid;
  80051a:	c088                	sw	a0,0(s1)
    for (i = 0; i < NCHILD; i++) {
  80051c:	0491                	addi	s1,s1,4
  80051e:	ff249be3          	bne	s1,s2,800514 <main+0x12>
    }

    for (i = 0; i < NCHILD; i++) {
        int st = 0;
        waitpid(pids[i], &st);
  800522:	4008                	lw	a0,0(s0)
  800524:	858a                	mv	a1,sp
    for (i = 0; i < NCHILD; i++) {
  800526:	0411                	addi	s0,s0,4
        int st = 0;
  800528:	c002                	sw	zero,0(sp)
        waitpid(pids[i], &st);
  80052a:	bbdff0ef          	jal	ra,8000e6 <waitpid>
    for (i = 0; i < NCHILD; i++) {
  80052e:	ff241ae3          	bne	s0,s2,800522 <main+0x20>
    }
    cprintf("BENCH DONE bench=short\n");
  800532:	00000517          	auipc	a0,0x0
  800536:	43e50513          	addi	a0,a0,1086 # 800970 <error_string+0x140>
  80053a:	b07ff0ef          	jal	ra,800040 <cprintf>
    return 0;
}
  80053e:	70e2                	ld	ra,56(sp)
  800540:	7442                	ld	s0,48(sp)
  800542:	74a2                	ld	s1,40(sp)
  800544:	7902                	ld	s2,32(sp)
  800546:	4501                	li	a0,0
  800548:	6121                	addi	sp,sp,64
  80054a:	8082                	ret
            unsigned int t_submit = gettime_msec();
  80054c:	b9fff0ef          	jal	ra,8000ea <gettime_msec>
  800550:	0005041b          	sext.w	s0,a0
            cprintf("BENCH SUBMIT pid=%d bench=short t=%u\n", getpid(), t_submit);
  800554:	b95ff0ef          	jal	ra,8000e8 <getpid>
  800558:	85aa                	mv	a1,a0
  80055a:	8622                	mv	a2,s0
  80055c:	00000517          	auipc	a0,0x0
  800560:	39c50513          	addi	a0,a0,924 # 8008f8 <error_string+0xc8>
  800564:	addff0ef          	jal	ra,800040 <cprintf>
            unsigned int t_start = gettime_msec();
  800568:	b83ff0ef          	jal	ra,8000ea <gettime_msec>
  80056c:	0005041b          	sext.w	s0,a0
            cprintf("BENCH START pid=%d t=%u\n", getpid(), t_start);
  800570:	b79ff0ef          	jal	ra,8000e8 <getpid>
  800574:	85aa                	mv	a1,a0
  800576:	8622                	mv	a2,s0
  800578:	00000517          	auipc	a0,0x0
  80057c:	3a850513          	addi	a0,a0,936 # 800920 <error_string+0xf0>
  800580:	ac1ff0ef          	jal	ra,800040 <cprintf>
            for (unsigned long k = 0; k < WORK_ITER; k++) {
  800584:	000496b7          	lui	a3,0x49
            volatile unsigned long x = 0;
  800588:	e002                	sd	zero,0(sp)
            for (unsigned long k = 0; k < WORK_ITER; k++) {
  80058a:	4401                	li	s0,0
  80058c:	3e068693          	addi	a3,a3,992 # 493e0 <_start-0x7b6c40>
                x += k ^ (k << 1);
  800590:	6702                	ld	a4,0(sp)
  800592:	00141793          	slli	a5,s0,0x1
  800596:	8fa1                	xor	a5,a5,s0
  800598:	97ba                	add	a5,a5,a4
  80059a:	e03e                	sd	a5,0(sp)
            for (unsigned long k = 0; k < WORK_ITER; k++) {
  80059c:	0405                	addi	s0,s0,1
  80059e:	fed419e3          	bne	s0,a3,800590 <main+0x8e>
            unsigned int t_end = gettime_msec();
  8005a2:	b49ff0ef          	jal	ra,8000ea <gettime_msec>
  8005a6:	0005049b          	sext.w	s1,a0
            cprintf("BENCH END pid=%d t=%u work=short iter=%d\n", getpid(), t_end, WORK_ITER);
  8005aa:	b3fff0ef          	jal	ra,8000e8 <getpid>
  8005ae:	85aa                	mv	a1,a0
  8005b0:	86a2                	mv	a3,s0
  8005b2:	8626                	mv	a2,s1
  8005b4:	00000517          	auipc	a0,0x0
  8005b8:	38c50513          	addi	a0,a0,908 # 800940 <error_string+0x110>
