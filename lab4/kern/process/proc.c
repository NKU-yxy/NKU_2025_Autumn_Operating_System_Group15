...
void proc_run(struct proc_struct *proc)
{
    if (proc != current)
    {
      ...
      //学号：2312189
        ...
        local_intr_restore(intr_flag);   // 传入之前保存的标志
    }
}
...
//学号：2312900
    ...
    // 2.分配内核栈    call setup_kstack to allocate a kernel stack for child process
    if (setup_kstack(proc) != 0)
    {
        goto bad_fork_cleanup_proc;
    }

    //3.复制内存管理信息（内核线程无需复制） call copy_mm to dup OR share mm according clone_flag
    if (copy_mm(clone_flags, proc) != 0)
    {
        goto bad_fork_cleanup_kstack;
    }
    //4. 复制上下文和陷阱帧   call copy_thread to setup tf & context in proc_struct
    copy_thread(proc, stack, tf);

    // 分配PID并设置进程关系   5. insert proc_struct into hash_list && proc_list
    proc->pid = get_pid();
    proc->parent = current;
    nr_process++;
    //  添加到进程列表和哈希表  6. call wakeup_proc to make the new child process RUNNABLE
    list_add(&proc_list, &proc->list_link);
    hash_proc(proc);
    //  7. 唤醒新进程  7. set ret vaule using child proc's pid
