    ...
    */
    ...
    if (page != NULL) {
        ...
        if (page->property > n) {
            ...
            SetPageProperty(p);
            // 插回到空闲链表，保持地址升序
            list_entry_t *le2 = &free_list;
            while ((le2 = list_next(le2)) != &free_list) {
                struct Page *pp = le2page(le2, page_link);
                if (p < pp) {
                    list_add_before(le2, &(p->page_link));
                    break;
                } else if (list_next(le2) == &free_list) {
                    list_add(le2, &(p->page_link));
                    break;
                }
            }
        }
...
best_fit_free_pages(struct Page *base, size_t n) {
    ...
    if (le != &free_list) {
        ...
        if (p + p->property == base) {  // 前一块与当前块连续
            p->property += base->property;
            ClearPageProperty(base);
            list_del(&(base->page_link));
            base = p;  // 更新 base 指针，用于下一步向后合并
        }
    }
